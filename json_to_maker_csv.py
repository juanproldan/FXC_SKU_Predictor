#!/usr/bin/env python
"""
Script to divide the Consolidado.json file into separate CSV files by maker.

This script processes the Consolidado.json file and creates a separate CSV file
for each maker, containing all the items for that maker.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("json_to_maker_csv.log"),
...
MAKER_DIR = os.path.join(OUTPUT_DIR, "maker_data")
...
def process_json_by_maker(input_path, output_dir):
    ...
    try:
        ...
        summary_path = os.path.join(output_dir, "maker_summary.csv")
        with open(summary_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for maker, rows in sorted(maker_rows.items()):
                ...
                maker_path = os.path.join(output_dir, f"{clean_name}.csv")
                
                with open(maker_path, 'w', newline='', encoding='utf-8') as mf:
                    writer_maker = csv.DictWriter(mf, fieldnames=["maker", "series", "model", "descripcion", "referencia"], quoting=csv.QUOTE_ALL)
...
def main():
    try:
        logger.info("Starting JSON to maker CSV conversion")
