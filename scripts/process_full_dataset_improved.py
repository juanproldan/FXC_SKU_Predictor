...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("process_full_dataset_improved.log"),
...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'Fuente_Json_Consolidado', 'clean_training_data.csv')
    ...
    try:
        ...
        for entry_idx, entry in enumerate(tqdm(data, desc="Processing entries")):
            ...
            for item in items:
                ...
                # Skip items missing required fields or with empty SKUs
                if not all([referencia, descripcion]) or referencia.strip() == "":
                    if referencia.strip() == "":
                        empty_skus += 1
                    else:
                        skipped_items += 1
        ...
        logger.info(f"Skipped {skipped_items} items due to missing required fields")
        ...
        # Check for any remaining empty SKUs
        empty_skus_in_output = sum(1 for sku in sku_counts.keys() if sku.strip() == "")
        if empty_skus_in_output > 0:
            logger.warning(f"Warning: Output still contains {empty_skus_in_output} empty SKUs")
        else:
            logger.info("No empty SKUs in the output - cleaning was successful")
...
def main():
    try:
        logger.info("Starting improved full dataset processing")
